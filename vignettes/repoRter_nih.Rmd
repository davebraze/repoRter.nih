...
```{r, echo = FALSE, message = FALSE, fig.width=6, fig.height=4, fig.align='center', cache=TRUE}
library(tibble)
...
res <- get_nih_data(req,
                    flatten_result = TRUE)
res %>%
  group_by(covid_response) %>%
  summarise(total_awards = sum(award_amount) / 1e6) %>%
  ungroup() %>%
  arrange(desc(covid_response)) %>%
  mutate(prop = total_awards / sum(total_awards),
         csum = cumsum(prop),
         ypos = csum - prop/2 ) %>%
  ggplot(aes(x = "", y = prop, fill = covid_response)) +
...
Yale is busy, but it is not the only org matching our search.

```{r}
res %>% pull(organization_org_name) %>% unique()
```
...
res %>%
  ...
  ggrepel::geom_text_repel(aes(label =
                                 paste0(scales::dollar(total_awards,
                                                       accuracy = 1,
                                                       suffix = "M"),
                                        "\n", scales::percent(prop, accuracy = .01)),
                               y = ypos),
                           show.legend = FALSE,
                           nudge_x = .8) +
  coord_polar(theta ="y") +
...
res %>% slice(4) %>% mutate(abstract_text = gsub("\\n", " ", abstract_text)) %>% print
```
