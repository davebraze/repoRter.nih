...
1. Calculate quantiles for the sample distribution of the `award_amount` column
...
1. Iterate over your quantiles to make separate requests, passing the endpoints of each quantile to `award$award_amount_range` criteria
- Wait til the end to flatten results since some columns may flatten differently on smaller individual result sets creating problems with combining them
1. Bind your list of results together
1. Flatten the complete result set if desired
